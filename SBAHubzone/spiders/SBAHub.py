    ...
    def parse(self, response):
        ...
        if batch.recs:
            yield scrapy.Request(
                ...
                dont_filter=True,
                callback=self.dispatch_batch_searches,
                cb_kwargs={"batch": batch},
            )

    def collect_businesses(self, response, search):
        table = response.css("table#ProfileTable tbody")
        rows = table.css("tr")
        businesses = []
        for row in rows:
            business = (
                row.css("td a::text").get(),
                row.css("td a::attr(href)").get(),
                row.css("td a::attr(href)").re_first(r".*\?.*=(.*)$"),
            )
            businesses.append(business)
        self.db.insert_businesses(businesses)
        self.db.update_search_is_searched(search["search_id"])
